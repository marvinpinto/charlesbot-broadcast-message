...
help:
	@echo '-----------------------------------------'
	@echo ' Charlesbot Broadcast Message Plugin make targets'
	@echo '-----------------------------------------'
	@echo 'help: This help'
...
checkstyle: install
	$(ENV)/bin/flake8 --max-complexity 10 charlesbot_broadcast_message
...
test: install
	$(ENV)/bin/nosetests \
		-v \
		--with-coverage \
		--cover-package=charlesbot_broadcast_message \
		tests

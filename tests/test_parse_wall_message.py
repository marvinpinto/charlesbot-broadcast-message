...
class TestParseWallMessage(asynctest.TestCase):
    ...
    def setUp(self):
        patcher1 = patch('charlesbot_broadcast_message.broadcast_message.BroadcastMessage.seed_initial_data')  # NOQA
        self.addCleanup(patcher1.stop)
        self.mock_seed_initial_data = patcher1.start()

        patcher4 = patch('charlesbot_broadcast_message.broadcast_message.BroadcastMessage.send_broadcast_message')  # NOQA
        self.addCleanup(patcher4.stop)
        self.mock_send_broadcast_message = patcher4.start()

        patcher5 = patch('charlesbot_broadcast_message.broadcast_message.SlackUser')  # NOQA
